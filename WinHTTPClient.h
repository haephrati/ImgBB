...
static wchar_t *SZ_Client = (wchar_t*)L"WinHttpClient";
...
public:
    ...
    inline bool SetUserClient(const wstring &userClient);
...
bool WinHttpClient::SendHttpRequest(const wstring &httpVerb, bool disableAutoRedirect)
{
	...
	else if (_wcsicmp(verb.c_str(), L"DELETE") == 0)
	{
		verb = L"DELETE";
	}
    else
    ...
    if (m_sessionHandle == NULL)
    {
        m_sessionHandle = ::WinHttpOpen(m_userClient.c_str(),  
            ...
            if (hRequest != NULL)
            {
                ...
                while (!bGetReponseSucceed && iRetryTimes++ < INT_RETRYTIMES)
                {
                    ...
                    if (::WinHttpSendRequest(hRequest,
                                             ...
                                             0,
                                             m_pDataToSend,//WINHTTP_NO_REQUEST_DATA,
                                             m_dataToSendSize,//0,
                                             m_dataToSendSize,//0,
                                             NULL))
                    {
...
bool WinHttpClient::SetUserClient(const wstring &userClient)
{
